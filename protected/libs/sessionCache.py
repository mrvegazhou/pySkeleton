...
class SessionManager(object):
    ...
    def set(self, request_handler, session):
        ...
        session_data = pickle.dumps(session.items() if session.items() else dict(session.items()), pickle.HIGHEST_PROTOCOL)
        mc = memcache.Client(self.memcached_address, debug=0)
        mc.set(session.session_id, session_data, self.session_timeout, 0)

    def clear(self, request_handler, session):
        request_handler.clear_cookie(session.session_id)
        request_handler.clear_cookie(session.hmac_key)
        mc = memcache.Client(self.memcached_address, debug=0)
